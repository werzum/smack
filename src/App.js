...
class App extends Component {
  constructor() {
    ...
    this.state = {
      punchCounter: 10,
      soundArray: [
        { index: 0, name: "Left", sound: this.punch1, active: true },
  ...
  async startLoop() {
    ...
    let newQueue = [];
    let prevTimeOut = 0;
    let activeSounds = this.state.soundArray.filter(
    ...
    for (let i = 0; i < this.state.punchCounter; i++) {
      ...
      let timeOut = getRandomInt(
        prevTimeOut + this.state.slider_1_min * 10,
        prevTimeOut + this.state.slider_1_max * 10
      );
        ...
        <header className="App-header">
          <Grid container direction="column" alignContent="center" spacing={0}>
            ...
            <Grid item className={classes.item}>
              ...
              <Slider
                value={this.state.punchCounter}
                onChange={this.handlePunchCounterChange}
                defaultValue={this.state.punchCounter}
